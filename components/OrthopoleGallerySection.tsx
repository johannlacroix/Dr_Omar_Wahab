...
const ORTHOPOLE_IMAGES = [
  ...
  '/Cabinet/Int_Cabinet_Ortho_1920px.webp',
  '/Cabinet/Omar_Wahab_Orthopole_4_opti_800px.webp',
  '/Cabinet/Omar_Wahab_Salle d_attente_1_opti_800px.webp',
  '/Cabinet/Omar_Wahab_Salle d_attente_5_opti_800px.webp',
];
...
export function OrthopoleGallerySection() {
  ...
  const nextImage = () => {
    setCurrentIndex((prev) => (prev === ORTHOPOLE_IMAGES.length - 1 ? 0 : prev + 1));
  };
    ...
    <>
      <div className="bg-white rounded-2xl overflow-hidden shadow-xl border border-neutral-200">
        <button
          ...
          aria-label="Ouvrir la galerie Orthopôle"
        >
          <img
      ...
      {isOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Galerie photo Orthopôle"
        >
          <div
          ...
          >
            ...
            <div className="relative h-[72vh] min-h-[360px] flex items-center justify-center px-4 md:px-6">
              <img
                src={ORTHOPOLE_IMAGES[currentIndex]}
                alt={`Orthopôle - photo ${currentIndex + 1}`}
            ...
            <div className="px-4 py-3 text-center text-sm text-white/85 font-light">
              {currentIndex + 1} / {ORTHOPOLE_IMAGES.length}
            </div>
          </div>
        </div>
      )}
